...
def generate_synthetic_kaggle_data(target_dir="data"):
    ...
    df_main = pd.DataFrame(
        {
            ...
            "experience_level": np.random.choice(
                ["EN", "MI", "SE", "EX"], n_rows, p=[0.15, 0.25, 0.45, 0.15]
            ),
    ...
    for ind in industries:
        for status in job_statuses:
            ...
            base_risk = (
                65 if status == "Decreasing" else (25 if status == "Increasing" else 40)
            )
            if ind == "Manufacturing" or ind == "Agriculture":
                base_risk += 15
            if ind == "IT" or ind == "Finance":
                base_risk -= 10
            ...
            impact_rows.append(
                {
                    ...
                    "Job Status": status,
                    "Automation Risk (%)": max(
                        0, min(100, np.random.normal(base_risk, 12))
                    ),
                    "AI Impact Level": "High"
                    if base_risk > 60
                    else ("Moderate" if base_risk > 35 else "Low"),
                }
            )
    ...
    df_time.to_csv(
        os.path.join(target_dir, "ai_data_science_2020_2026.csv"), index=False
    )
