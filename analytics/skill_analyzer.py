...
def analyze_market_trends(data_path, timestamp=None):
    ...
    pd.DataFrame({"Concept": feature_names, "Score": tfidf_scores}).sort_values(
        ...
        for j, s2 in enumerate(top_skills_for_graph):
            ...
            count = corpus.apply(
                lambda x: (
                    1
                    if (
                        re.search(r"\b" + re.escape(s1) + r"\b", x)
                        and re.search(r"\b" + re.escape(s2) + r"\b", x)
                    )
                    else 0
                )
            ).sum()
    ...
    sns.barplot(
        x=list(group_counts.values()), y=list(group_counts.keys()), palette="magma"
    )
    ...
    nx.draw_networkx_edges(
        G, pos, width=[w / 2 for w in weights], edge_color="grey", alpha=0.5
    )
...
def generate_roadmap(groups, top_skills, timestamp=None):
    ...
    with open(roadmap_path, "w", encoding="utf-8") as f:
        ...
        f.write("Generated from analyzing latest job postings on ITviec.\n\n")

        f.write("## Phase 1: Core Mastery\n")
        f.write("- **Java Fundamentals**: Streams, Lambda, Multithreading.\n")
        f.write(
            "- **JVM Internals**: Memory Model, GC (Commonly asked in senior roles).\n\n"
        )

        f.write("## Phase 2: The Framework King\n")
        f.write(
            "- **Spring Boot**: The #1 requirement. Focus on REST APIs and Security.\n"
        )
        ...
        f.write("- Most mentioned architecture: **Microservices**\n")
        f.write("- Top companion skill: **Docker**\n")
