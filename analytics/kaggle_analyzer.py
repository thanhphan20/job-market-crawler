...
class KaggleMarketAnalyzer:
    ...
    def analyze_salary_distribution(self, df, dataset_name="Kaggle"):
        ...
        # Add labels
        plt.title(
            f"Global AI Salary Distribution Insights ({dataset_name})",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
        ...
        plt.axvline(
            median, color="red", linestyle="--", label=f"Median: ${median:,.0f}"
        )
        plt.legend()

        plt.tight_layout()
        save_path = os.path.join(
            self.output_dir, f"salary_dist_{dataset_name.lower()}.png"
        )
    ...
    def analyze_ai_impact(self, df):
        ...
        if "risk" not in df_clean.columns or "status" not in df_clean.columns:
            print(
                "[!] AI Impact dataset missing expected columns ('Automation Risk (%)' or 'Job Status'). Skipping."
            )
            return
        ...
        industry_risk = (
            df_clean.groupby(["industry", "status"])["risk"].mean().unstack().fillna(0)
        )

        sns.heatmap(industry_risk, annot=True, cmap="YlOrRd", fmt=".1f")

        plt.title(
            "AI Impact Matrix: Automation Risk by Industry & Job Status",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
    ...
    def analyze_skills_demand(self, df):
        ...
        plt.title(
            "Global AI Skills Demand Ranking (Kaggle Research)",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
    ...
    def run_all(self):
        ...
        if df_growth is not None and "work_year" in df_growth.columns:
            ...
            plt.title(
                "Global AI Salary Evolution (2020-2026 Forecast)",
                fontsize=16,
                fontweight="bold",
            )
    ...
    def get_processed_data(self):
        ...
        if df_impact is not None:
            ...
            df_clean = df_impact.rename(
                columns={k: v for k, v in cols.items() if k in df_impact.columns}
            )
            if (
                "industry" in df_clean.columns
                and "status" in df_clean.columns
                and "risk" in df_clean.columns
            ):
                impact_summary = (
                    df_clean.groupby(["industry", "status"])["risk"]
                    .mean()
                    .reset_index()
                )
        ...
        if df_growth is not None and "work_year" in df_growth.columns:
            trends = (
                df_growth.groupby("work_year")["salary_in_usd"].median().reset_index()
            )
                ...
                data["skill_matrix"].append(
                    {
                        ...
                        ),  # Simulated growth based on trend
                    }
                )
        ...
        if df_main is not None and "industry" in df_main.columns:
            ...
            for industry, count in shares.items():
                data["market_share"].append({"name": industry, "value": int(count)})
