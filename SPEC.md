...
2. `run_agentic_analysis()`
   ...
   - `_export_dashboard_json(merged)` — builds the **data contract** (§4) and writes it to `SYNC_DIR/intelligence.json` and `public/data/intelligence.json`, then cloud-syncs. The `skills` vector comes from `_extract_skill_matrix()`: it prefers the real survey skills (`required_skills` / SO `LanguageHaveWorkedWith`) via `_skills_from_survey()`, and falls back to mining local job titles (`SKILL_KEYWORDS`) via `_skills_from_titles()`. Automation risk falls back to a role-keyword heuristic when no impact dataset is loaded.
